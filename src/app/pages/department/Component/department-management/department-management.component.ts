...
import { DepartmentsClient, DepartmentDto, DepartmentListDto, CreateDepartmentDto, CreateDepartmentListCommand } from '../../../../core/services/api-client';
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, TableModule, ButtonModule, ToastModule, InputTextModule, TreeModule, FileUploadModule, FontAwesomeModule],
...
export class DepartmentManagementComponent implements OnInit, OnDestroy {
        ...
        try {
            const importConfig = {
                ...
                processData: async (data: any[]) => {
                    const departments = data.map((entity) =>
                    ...
                    await lastValueFrom(this.departmentsClient.createDepartmentList(command));
                },
            ...
            this.messageService.add({
                severity: 'success',
                summary: 'نجاح',
                detail: 'تم استيراد البيانات بنجاح.'
            });
        } catch (error) {
            this.messageService.add({
                severity: 'error',
    ...
    filterTable(): void {
        ...
        const filterTextLower = this.filterText.toLowerCase();
        this.filteredDepartments = this.departments.filter((dept) => dept.name?.toLowerCase().includes(filterTextLower) || dept.parentDepartment?.toLowerCase().includes(filterTextLower));
        this.applySort();
    }
